...
def init_step_cache(image_dir):
    ...
    for image_name in files:
        ...
        # _, binary_image = cv2.threshold(image, 128, 255, cv2.THRESH_BINARY)
        # cache[image_name.replace(".png", "")] = binary_image
        cache_step[image_name.replace(".png", "")] = image_path
...
def match_step_template_in_directory(screenshot, threshold=0.7):
    if screenshot is None or len(screenshot) == 0:  # 确保截图不为空
        return None
    for key in cache_step.keys():
        path = cache_step[key]
        if comparator.template_compare(path, screenshot=screenshot, match_threshold=threshold):
            return key
    return None
