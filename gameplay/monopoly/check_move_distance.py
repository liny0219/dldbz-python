...
from gameplay.monopoly.ocr import is_number, match_step_template_in_directory, ocr_number, write_ocr_log
...
def check_move_distance(monopoly: Monopoly):
    ...
    try:
        ...
        if not is_number(number):
            app_data.update_ui(f"check-检查步数失败启动备用图片识别")
            start_time = time.time()
            currentshot = monopoly.shot()
            current_image = currentshot[y:y+height, x:x+width]
            number = match_step_template_in_directory(currentshot)
            write_ocr_log(number, current_image, 'move_distance_faild')
            app_data.update_ui(f"check-检查结束耗时:{time.time() - start_time}")
            if is_number(number):
